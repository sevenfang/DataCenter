...
class PublishProcess:
    ...
    def check_svn_f_dir(self, svn_path):
        svn_path = str(svn_path)
        # 拆分所有父类
        svn_f_p_list = []
        while (svn_path.__contains__("/")):
        ...
        for i in range(svn_f_p_list_index - 1, -1, -1):
            if not self.addDir(svn_f_p_list[i], True):
                return False
    ...
    def publish_process(self, publish_type, publish_version, svn_path, publish_author):
        user_path = SysInitData.getPathByUserid(publish_author)
        if user_path is None or not svn_path.startswith(user_path):
            return False, '无权限', ''
        else:
            self.publish_author = publish_author
        try:
            ...
            if publish_type == 0:
                ...
                if not self.check_svn_f_dir(svn_path):
