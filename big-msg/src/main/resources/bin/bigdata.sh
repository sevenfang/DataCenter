#!/bin/bash
###########程序特定配置#############
MAIN_CLASS=com.chezhibao.bigdata.BigMsgApplication
APPLICATION_NAME=bigMsg
...
if [ ${OPT}x == "start"x ]; then
    ./startup.sh ${START_PARAMS} -m ${MAIN_CLASS}
elif [ ${OPT}x == "stop"x ]; then
    ./stop.sh ${APPLICATION_INFO_PATH}
elif [ ${OPT}x == "restart"x ]; then
    ./restart.sh ${APPLICATION_INFO_PATH} ${START_PARAMS} -m ${MAIN_CLASS}
