...
import com.sohu.cache.util.BigdataResult;
...
public class RedisClientController {
     ...
     * 获取所有的实例信息
     */
    @RequestMapping(value = "/redis/apps")
    @ResponseBody
    public Object getAllApps() {
        try {
            List<AppDesc> allAppDesc = appService.getAllAppDesc();
            Map<String, Map<String,Object>> result = new HashMap<>(allAppDesc.size());
            if (ObjectUtils.isEmpty(allAppDesc)) {
                return BigdataResult.build(400, "未查询到应用信息!!");
            }
            for (AppDesc appDesc : allAppDesc) {
                int status = appDesc.getStatus();
                if (status == AppCheckEnum.APP_ALLOCATE_RESOURCE.value()) {
                    Map<String,Object> appInfo = new HashMap<>();
                    String name = appDesc.getName();
                    result.put(name,appInfo);
                    String appKey = appDesc.getAppKey();
            ...
            return BigdataResult.ok(result);
        } catch (Exception e) {
            return BigdataResult.build(400, "未查询到应用信息!!" + e.getMessage());
        }
    }
