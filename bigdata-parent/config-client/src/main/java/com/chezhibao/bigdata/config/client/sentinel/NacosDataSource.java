...
public class NacosDataSource<T> extends AbstractDataSource<String, T> {
     ...
     *
     * @param properties config params of Nacos, cannot be empty
    ...
    public NacosDataSource(final Properties properties, final String groupId, final String dataId,
        ...
        super(parser);
        if (properties==null || properties.size()==0 || StringUtil.isBlank(groupId) || StringUtil.isBlank(dataId)) {
            throw new IllegalArgumentException(String.format("Bad argument: serverAddr=[%s], groupId=[%s], dataId=[%s]",
                    properties, groupId, dataId));
        }
    ...
    private void initNacosListener(Properties properties) {
        try {
